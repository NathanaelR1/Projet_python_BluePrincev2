    ...
    def __init__(self, nom, image, portes,type_de_piece, cout_gemmes=0, objets=None, effet=None, rarete=0, condition=None,
        ...
        if interactions:
            for inter in interactions:
                quantite = inter.get("quantite", 1)
                base = inter.copy()
                base.pop("quantite", None)
                for _ in range(quantite):
                    self.interactions.append(base.copy())
        ...
        for direction in ["haut", "droite", "bas", "gauche"]:
            if self.niveaux_portes[direction] is None:
                ...
                if rangee == 8:
                    ...
                    niveau = random.choice([0,0,1,0])
                elif  1 <= rangee <= 4:
                    niveau = random.choice([0,1, 2])
                else:
                    niveau = 2
